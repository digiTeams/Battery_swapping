...
class OptimumDRL:
    ...
    def BILP_costs(self, bundling, scale_):
        ...
        # qvals[(b, j)] = reward + gamma*V_{t+1};
        qvals = self.bundle_costs(bundling, agent_news, agent_clusters, scale_)

        #Empty bundle assignment costs:
        #cvals[(k, j)] = reward + gamma*V_{t+1};
    ...
    def bundle_costs(self, bundling, agent_news, agent_clusters, scale_):
        ...
        for (b, j) in bundling.BS_pairs:
            ...
            for k in range(num_levels):
                ...
                #simulate queue process: next_obs, done, revenue, utilities
                next_obs, done, revenue, _ = agent.simulate_bundle(self.environment.time_step,
    ...
    def action_BILP(self, bundling, qvals, cvals):
        t0 = time.perf_counter()
        base_stocks = RL_params['base_stocks']
        index = {req.id: i   for i, req in enumerate(self.environment.new_demand)}
        ...
        if env_params['test_mode'] and self.environment.new_demand:
            print(f'BILP optimization under OptDRL: status {sol_status}; build time {build_time:.2f} minutes; '
                  f'runtime {BILP.Runtime:.2f} seconds')
            print(f'OptDRL station-bundle assignments: {bundle_assign}.')
                ...
                if not feasible_stations:
                    ...
                    for j, sid in enumerate(feasible_stations):
                        ...
                        for rid in bundle:
                            ...
                            travel_time = env_params['travel_time_factor'] * (
                                        dist / env_params['vehicle_speed']) * 60  # minutes
    ...
    def DRL_schedule(self, bundling, train_flag, scale_):
        ...
        self.opt_act += 1
        qvals, cvals = self.BILP_costs(bundling, scale_)
        if self.environment.new_demand:
            action, _ = self.action_BILP(bundling, qvals, cvals)
        else:
    ...
    # Custom Huber (smooth L1) loss:
    def huber_loss(self, pred: torch.Tensor, target: torch.Tensor,
                   reduction: str) -> torch.Tensor:
        """
          - quadratic for |r| <= delta, linear beyond.
          - r = pred - target
        """
        error = pred - target
        abs_error = torch.abs(error)

        # quadratic term: min(|r|, delta)
        quadratic = torch.minimum(abs_error, torch.tensor(self.delta, device=abs_error.device))
        # linear term: |r| - quadratic
    ...
    def scheduling(self, bundling, train_flag, scale_, episode, seed):
        ...
        #print(f'***************Start the distributed DRL policy with {num_cores} CPU cores. ***************')
        ...
        if not train_flag:
            ...
            self.environment.request_records(f'w{w}DRL', episode)

        # Summary
        print(f"*************** optDRL(w={env_params['weight']}) policy in episode {episode} *************** \n"
